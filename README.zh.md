...
<details>
<summary><b>自有的文字排版</b></summary>

选中一个词，菜单里会在粗体和斜体旁边出现「字号」「变暗」「彩虹」。字号两头都有
限制：既不会只有两像素，也不会占满整屏。

Telegram 的排版类型是服务端上的封闭列表，加不进新东西。所以排版信息直接藏在消息
文本里，用不可见字符表示，由分支自己解析。没有装分支的人看到的是普通文本，外加
消息末尾的一行链接。

每一种都可以单独关闭。旁边还有「带格式复制」：Telegram 原本的复制只给纯文本，这个
会保留粗体、斜体、链接和我们自己的排版。
</details>
